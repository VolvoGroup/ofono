...
static void cint_cpbr_notify(GAtResult *result, gpointer user_data)
{
...
static void cint_read_entries_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct cb_data *cbd = user_data;
	struct ofono_phonebook *pb = cbd->user;
...
static void cint_read_entries(struct cb_data *cbd)
{
	struct ofono_phonebook *pb = cbd->user;
	struct pb_data *pbd = ofono_phonebook_get_data(pb);
	char buf[32];
	...
	if (g_at_chat_send_listing(pbd->chat, buf, cpbr_prefix,
					cint_cpbr_notify, cint_read_entries_cb,
					cbd, NULL) > 0)
		return;
...
static void cint_set_charset_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct cb_data *cbd = user_data;
	...
	cint_read_entries(cbd);
}

static void cint_read_charset_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct cb_data *cbd = user_data;
	struct ofono_phonebook *pb = cbd->user;
	...
	if (g_at_chat_send(pbd->chat, buf, none_prefix,
				cint_set_charset_cb, cbd, NULL) > 0)
		return;
...
static void cint_list_indices_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct cb_data *cbd = user_data;
	struct ofono_phonebook *pb = cbd->user;
	...
	if (g_at_chat_send(pbd->chat, "AT+CSCS?", cscs_prefix,
				cint_read_charset_cb, cbd, NULL) > 0)
		return;
...
static void cint_select_storage_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct cb_data *cbd = user_data;
	struct ofono_phonebook *pb = cbd->user;
	...
	if (g_at_chat_send(pbd->chat, "AT+CPBR=?", cpbr_prefix,
				cint_list_indices_cb, cbd, NULL) > 0)
		return;
...
static void cint_export_entries(struct ofono_phonebook *pb, const char *storage,
				ofono_phonebook_cb_t cb, void *data)
{
	...
	if (g_at_chat_send(pbd->chat, buf, none_prefix,
				cint_select_storage_cb, cbd, NULL) > 0)
		return;
...
static void cint_list_storages_cb(gboolean ok, GAtResult *result,
						gpointer user_data);
...
static gboolean cpbs_support_check(gpointer user_data)
{
	...
	if (g_at_chat_send(pbd->chat, "AT+CPBS=?", cpbs_prefix,
				cint_list_storages_cb, pb, NULL) > 0)
		return FALSE;
...
static void cint_list_charsets_cb(gboolean ok, GAtResult *result,
						gpointer user_data)
{
	struct ofono_phonebook *pb = user_data;
	struct pb_data *pbd = ofono_phonebook_get_data(pb);
...
static void cint_list_charsets(struct ofono_phonebook *pb)
{
	struct pb_data *pbd = ofono_phonebook_get_data(pb);

	if (g_at_chat_send(pbd->chat, "AT+CSCS=?", cscs_prefix,
				cint_list_charsets_cb, pb, NULL) > 0)
		return;

	phonebook_not_supported(pb);
}

static int cint_phonebook_probe(struct ofono_phonebook *pb, unsigned int vendor,
				void *data)
{
	GAtChat *chat = data;
	struct pb_data *pbd;
	...
	cint_list_charsets(pb);

	return 0;
}

static void cint_phonebook_remove(struct ofono_phonebook *pb)
{
	struct pb_data *pbd = ofono_phonebook_get_data(pb);
...
static struct ofono_phonebook_driver driver = {
	.name		= "cinterionmodem",
	.probe		= cint_phonebook_probe,
	.remove		= cint_phonebook_remove,
	.export_entries	= cint_export_entries
};

void cint_phonebook_init(void)
{
	ofono_phonebook_driver_register(&driver);
}

void cint_phonebook_exit(void)
{
