...
struct cinterion_cbs_data {
	GAtChat *chat;
	unsigned int modem;
};

static void cint_cbm_notify(GAtResult *result, gpointer user_data)
{
...
static void cint_cscb_set_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
...
static void cint_cbs_set_topics(struct ofono_cbs *cbs, const char *topics,
				ofono_cbs_set_cb_t cb, void *user_data)
{
	struct cinterion_cbs_data *cd = ofono_cbs_get_data(cbs);
	...
	/* Cinterion modems require that the topics are reset to 0 first */
	g_at_chat_send(cd->chat, "AT+CSCB=0", none_prefix, NULL, NULL, NULL);

	buf = g_strdup_printf("AT+CSCB=0,\"%s\"", topics);

	id = g_at_chat_send(cd->chat, buf, none_prefix,
				cint_cscb_set_cb, cbd, g_free);
...
static void cint_cbs_clear_topics(struct ofono_cbs *cbs,
				ofono_cbs_set_cb_t cb, void *user_data)
{
	struct cinterion_cbs_data *cd = ofono_cbs_get_data(cbs);
	struct cb_data *cbd = cb_data_new(cb, user_data);
	...
	if (g_at_chat_send(cd->chat, "AT+CSCB=0", none_prefix,
				cint_cscb_set_cb, cbd, g_free) > 0)
		return;
...
static void cint_cbs_register(gboolean ok, GAtResult *result, gpointer user)
{
	 ...
	 */
	g_at_chat_register(cd->chat, "+CBM:", cint_cbm_notify, TRUE, cbs, NULL);

	ofono_cbs_register(cbs);
}

static void cint_cscb_support_cb(gboolean ok, GAtResult *result, gpointer user)
{
	struct ofono_cbs *cbs = user;
	struct cinterion_cbs_data *cd = ofono_cbs_get_data(cbs);
	gint range[2];
...
static int cint_cbs_probe(struct ofono_cbs *cbs, unsigned int vendor,
				void *data)
{
	GAtChat *chat = g_at_chat_clone(data);
	struct cinterion_cbs_data *cd = g_try_new0(struct cinterion_cbs_data, 1);
	cd->chat = chat;
	cd->modem = vendor;

	ofono_cbs_set_data(cbs, cd);

	g_at_chat_send(cd->chat, "AT+CSCB=?", cscb_prefix,
			cint_cscb_support_cb, cbs, NULL);

	return 0;
}

static void cint_cbs_remove(struct ofono_cbs *cbs)
{
	struct cinterion_cbs_data *cd = ofono_cbs_get_data(cbs);
...
static struct ofono_cbs_driver driver = {
	.name = "cinterionmodem",
	.probe = cint_cbs_probe,
	.remove = cint_cbs_remove,
	.set_topics = cint_cbs_set_topics,
	.clear_topics = cint_cbs_clear_topics,
};

void cint_cbs_init(void)
{
	ofono_cbs_driver_register(&driver);
}

void cint_cbs_exit(void)
{
