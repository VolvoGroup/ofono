...
static void cinterion_context_act_cb(gboolean ok, GAtResult *result,
					gpointer user_data)
{
	struct ofono_gprs_context *gc = user_data;
	struct cint_gprs_context_data *gcd = ofono_gprs_context_get_data(gc);
	...
	switch (gcd->modem) {
	case CINTERION_ALS3:
		cinterion_swwan(ok, result, user_data);
		return;
	default:
		break;
	}
				...
				ofono_gprs_context_cb_t cb, void *data)
{
	...
	/* TODO: add authentication step AT$QCPDPP=1,1,"password","username" */

	switch (gcd->modem) {
	case CINTERION_ALS3:
		/* TODO: Support both adapters? */
		snprintf(buf, sizeof(buf) - 1,
				"AT^SWWAN=1,%u,1", ctx->cid);
		break;
	default:
		goto error;
		break;
	...
	if (gcd->state == STATE_SET_APN &&
		g_at_chat_send(gcd->chat, buf, none_prefix,
				cinterion_context_act_cb, gc, NULL) > 0) {
		return;
	}
					...
					ofono_gprs_context_cb_t cb, void *data)
{
	...
	if (g_at_chat_send(gcd->chat, buf, none_prefix,
				cinterion_context_deact_cb, gc, NULL) > 0)
		return;
