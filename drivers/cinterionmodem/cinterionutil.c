...
#include "cinterionutil.h"
#include "modemmodel.h"

static const char *cpin_prefix[] = { "+CPIN:", NULL };

struct cint_util_sim_state_query {
	GAtChat *chat;
...
GSList *cint_util_parse_clcc(GAtResult *result, unsigned int *ret_mpty_ids)
{
	GAtResultIter iter;
	GSList *l = NULL;
	...
	while (g_at_result_iter_next(&iter, "+CLCC:")) {
		...
		l = g_slist_insert_sorted(l, call, cint_util_call_compare);
					...
					unsigned int modem)
{
	...
	/* Don't get anything else when unregistered */
	if (s != 1 && s != 5)
		goto out;
	...
	/* Not all Cinterion modems report bearer with CGREG */
	if (g_at_result_iter_next_number(&iter, &t) == FALSE)
		goto out;
				...
				unsigned int modem)
{
	...
	while (g_at_result_iter_next(&iter, prefix)) {
		...
		/* Not all Cinterion modems report bearer with CREG */
		if(g_at_result_iter_next_number(&iter, &t) == FALSE)
			goto out;
...
gboolean cint_util_parse_sms_index_delivery(GAtResult *result,
					const char *prefix,
					enum cint_util_sms_store *out_st,
					int *out_index)
{
	GAtResultIter iter;
	const char *strstore;
	enum cint_util_sms_store st;
	int index;
	...
	if (g_str_equal(strstore, "ME"))
		st = CINT_UTIL_SMS_STORE_ME;
	else if (g_str_equal(strstore, "SM"))
		st = CINT_UTIL_SMS_STORE_SM;
	else if (g_str_equal(strstore, "SR"))
		st = CINT_UTIL_SMS_STORE_SR;
	else if (g_str_equal(strstore, "BM"))
		st = CINT_UTIL_SMS_STORE_BM;
	else
		return FALSE;
...
static gboolean cint_util_charset_string_to_charset(const char *str,
					enum cint_util_charset *charset)
{
	if (!g_strcmp0(str, "GSM"))
		*charset = CINT_UTIL_CHARSET_GSM;
	else if (!g_strcmp0(str, "HEX"))
		*charset = CINT_UTIL_CHARSET_HEX;
	else if (!g_strcmp0(str, "IRA"))
		*charset = CINT_UTIL_CHARSET_IRA;
	else if (!g_strcmp0(str, "PCCP437"))
		*charset = CINT_UTIL_CHARSET_PCCP437;
	else if (!g_strcmp0(str, "PCDN"))
		*charset = CINT_UTIL_CHARSET_PCDN;
	else if (!g_strcmp0(str, "UCS2"))
		*charset = CINT_UTIL_CHARSET_UCS2;
	else if (!g_strcmp0(str, "UTF-8"))
		*charset = CINT_UTIL_CHARSET_UTF8;
	else if (!g_strcmp0(str, "8859-1"))
		*charset = CINT_UTIL_CHARSET_8859_1;
	else if (!g_strcmp0(str, "8859-2"))
		*charset = CINT_UTIL_CHARSET_8859_2;
	else if (!g_strcmp0(str, "8859-3"))
		*charset = CINT_UTIL_CHARSET_8859_3;
	else if (!g_strcmp0(str, "8859-4"))
		*charset = CINT_UTIL_CHARSET_8859_4;
	else if (!g_strcmp0(str, "8859-5"))
		*charset = CINT_UTIL_CHARSET_8859_5;
	else if (!g_strcmp0(str, "8859-6"))
		*charset = CINT_UTIL_CHARSET_8859_6;
	else if (!g_strcmp0(str, "8859-C"))
		*charset = CINT_UTIL_CHARSET_8859_C;
	else if (!g_strcmp0(str, "8859-A"))
		*charset = CINT_UTIL_CHARSET_8859_A;
	else if (!g_strcmp0(str, "8859-G"))
		*charset = CINT_UTIL_CHARSET_8859_G;
	else if (!g_strcmp0(str, "8859-H"))
		*charset = CINT_UTIL_CHARSET_8859_H;
	else
		return FALSE;
...
gboolean cint_util_parse_cscs_supported(GAtResult *result, int *supported)
{
	...
	while (g_at_result_iter_next_string(&iter, &str)) {
		if (cint_util_charset_string_to_charset(str, &charset))
			*supported |= charset;
	}
				...
				enum cint_util_charset *charset)
{
	...
	if (g_at_result_iter_next_string(&iter, &str))
		return cint_util_charset_string_to_charset(str, charset);

	return FALSE;
}

static const char *cint_util_fixup_return(const char *line, const char *prefix)
{
				...
				const char **out_attr)
{
	...
	if (out_attr)
		*out_attr = cint_util_fixup_return(line, prefix);
...
{
	struct cint_util_sim_state_query *req = userdata;
...
struct cint_util_sim_state_query *cint_util_sim_state_query_new(GAtChat *chat,
						...
						GDestroyNotify destroy)
{
	struct cint_util_sim_state_query *req;

	req = g_new0(struct cint_util_sim_state_query, 1);
