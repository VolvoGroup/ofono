					...
					const char *num, int num_type, int clip)
{
	...
	d->calls = g_slist_insert_sorted(d->calls, call, cint_util_call_compare);
...
static void clcc_poll_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	...
	calls = cint_util_parse_clcc(result, NULL);
...
static void cint_dial(struct ofono_voicecall *vc,
...
static void cint_template(const char *cmd, struct ofono_voicecall *vc,
...
static void cint_answer(struct ofono_voicecall *vc,
			ofono_voicecall_cb_t cb, void *data)
{
	cint_template("ATA", vc, generic_cb, 0, cb, data);
}

static void cint_hangup(struct ofono_voicecall *vc,
			ofono_voicecall_cb_t cb, void *data)
{
	/* Hangup active call */
	cint_template("AT+CHUP", vc, generic_cb, 0x3f, cb, data);
}
...
static void clcc_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	...
	vd->calls = cint_util_parse_clcc(result, NULL);

	for (l = vd->calls; l; l = l->next)
		ofono_voicecall_notify(vc, l->data);
}

static void cint_hold_all_active(struct ofono_voicecall *vc,
				ofono_voicecall_cb_t cb, void *data)
{
	cint_template("AT+CHLD=2", vc, generic_cb, 0, cb, data);
}

static void cint_release_all_held(struct ofono_voicecall *vc,
				ofono_voicecall_cb_t cb, void *data)
{
	unsigned int held_status = 1 << CALL_STATUS_HELD;
	cint_template("AT+CHLD=0", vc, generic_cb, held_status, cb, data);
}

static void cint_set_udub(struct ofono_voicecall *vc,
			ofono_voicecall_cb_t cb, void *data)
{
	unsigned int incoming_or_waiting =
		(1 << CALL_STATUS_INCOMING) | (1 << CALL_STATUS_WAITING);

	cint_template("AT+CHLD=0", vc, generic_cb, incoming_or_waiting,
			cb, data);
}

static void cint_release_all_active(struct ofono_voicecall *vc,
					ofono_voicecall_cb_t cb, void *data)
{
	cint_template("AT+CHLD=1", vc, generic_cb, 0x1, cb, data);
}

static void cint_release_specific(struct ofono_voicecall *vc, int id,
				ofono_voicecall_cb_t cb, void *data)
{
	struct voicecall_data *vd = ofono_voicecall_get_data(vc);
...
static void cint_private_chat(struct ofono_voicecall *vc, int id,
				ofono_voicecall_cb_t cb, void *data)
{
	char buf[32];

	snprintf(buf, sizeof(buf), "AT+CHLD=2%d", id);
	cint_template(buf, vc, generic_cb, 0, cb, data);
}

static void cint_create_multiparty(struct ofono_voicecall *vc,
					ofono_voicecall_cb_t cb, void *data)
{
	cint_template("AT+CHLD=3", vc, generic_cb, 0, cb, data);
}

static void cint_transfer(struct ofono_voicecall *vc,
			ofono_voicecall_cb_t cb, void *data)
{
	/* Held & Active */
	...
	cint_template("AT+CHLD=4", vc, generic_cb, transfer, cb, data);
}

static void cint_deflect(struct ofono_voicecall *vc,
			const struct ofono_phone_number *ph,
			ofono_voicecall_cb_t cb, void *data)
{
	char buf[128];
	...
	cint_template(buf, vc, generic_cb, incoming_or_waiting, cb, data);
}
...
static void cint_send_dtmf(struct ofono_voicecall *vc, const char *dtmf,
			ofono_voicecall_cb_t cb, void *data)
{
	struct voicecall_data *vd = ofono_voicecall_get_data(vc);
	struct cb_data *cbd = cb_data_new(cb, data);
...
static void ring_notify(GAtResult *result, gpointer user_data)
{
	...
	if (g_slist_find_custom(vd->calls,
				...
				cint_util_call_compare_by_status))
		return;
...
static void cint_voicecall_initialized(gboolean ok, GAtResult *result,
					gpointer user_data)
{
	struct ofono_voicecall *vc = user_data;
	struct voicecall_data *vd = ofono_voicecall_get_data(vc);
...
static int cint_voicecall_probe(struct ofono_voicecall *vc, unsigned int vendor,
				void *data)
{
	GAtChat *chat = data;
	...
	/* HACK: Activate speaker phone mode */
	...
	g_at_chat_send(vd->chat, "AT+CCWA=1", NULL,
				cint_voicecall_initialized, vc, NULL);

	return 0;
}

static void cint_voicecall_remove(struct ofono_voicecall *vc)
{
	struct voicecall_data *vd = ofono_voicecall_get_data(vc);
...
static struct ofono_voicecall_driver driver = {
	.name			= "cinterionmodem",
	.probe			= cint_voicecall_probe,
	.remove			= cint_voicecall_remove,
	.dial			= cint_dial,
	.answer			= cint_answer,
	.hangup_all		= cint_hangup,
	.hold_all_active	= cint_hold_all_active,
	.release_all_held	= cint_release_all_held,
	.set_udub		= cint_set_udub,
	.release_all_active	= cint_release_all_active,
	.release_specific	= cint_release_specific,
	.private_chat		= cint_private_chat,
	.create_multiparty	= cint_create_multiparty,
	.transfer		= cint_transfer,
	.deflect		= cint_deflect,
	.swap_without_accept	= NULL,
	.send_tones		= cint_send_dtmf
};

void cint_voicecall_init(void)
{
	ofono_voicecall_driver_register(&driver);
}

void cint_voicecall_exit(void)
{
